...
static void inference_and_performance(
	...
	SimpleYolo::Mode mode, 
	SimpleYolo::Type type, const string& model_name)
{
...
static void test(SimpleYolo::Type type, SimpleYolo::Mode mode, const string& model)
{
	...
	cout << "model.c_str():"<<model.c_str() << endl;
    printf("===================== test %s %s %s ==================================\n", SimpleYolo::type_name(type), mode_name, name);

    //判断模型是否存在，不存在则远程下载
	if (!requires_model(name))
	{
		std::cout << "Can not find model file:" << model.c_str() << std::endl;
		return;
	}
...
void direct_test()
{
    ...
    if(!exists(model_file))
	{
		cout << " Find model file success!" << endl;
		if (!SimpleYolo::compile(mode, type, 1, onnx_file, model_file, 1 << 30, "inference"))
		{
	...
	cout << "direct_test() finished" << endl;
	cv::imshow("des", image);
	cv::waitKey(0);
}
...
void my_test()
{
	...
	auto image = cv::imread("inference/gril.jpg");
	double start = cv::getTickCount();//计时，获取cpu跳数
	auto objs = yolo->commit(image).get();
	...
	double end = cv::getTickCount();//计时，获取cpu跳数
	double sec = (end - start) / Frequency;
	cout <<" inference  sec is : " << sec << endl;
